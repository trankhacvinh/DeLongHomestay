(function () {
    ...
    function sourceValue(target) { return target?.matches?.('[data-nav-id]') ? target.dataset.navSourceUrl || '' : target?.dataset?.tokenSource || ''; }
    function setSourceValue(target, value) { if (target?.matches?.('[data-nav-id]')) target.dataset.navSourceUrl = value; else if (target?.dataset) target.dataset.tokenSource = value; }
    ...
    async function resolveRoomDestination(room) {
        ...
        return { kind: 'room', label: room.name || room.code || 'Phòng', detail: `${room.propertyName || ''}${room.code ? ` · ${room.code}` : ''}`.replace(/^ · | · $/g, ''), url, source: url };
    }
    ...
    async function loadDestinations() {
        ...
        destinationPromise = (async () => {
            ...
            const destinations = systemLinks.map(link => ({ kind: 'system', label: link.label, detail: resolveToken(link.token), url: resolveToken(link.token), source: link.token, icon: link.icon }));
            ...
            const roomResults = await Promise.allSettled((Array.isArray(context.rooms) ? context.rooms.slice(0, 100) : []).map(resolveRoomDestination));
            roomResults.forEach(result => { if (result.status === 'fulfilled' && result.value) destinations.push(result.value); });
            ...
            try {
                ...
                (Array.isArray(editorial?.posts) ? editorial.posts : []).filter(post => post.isPublished !== false).slice(0, 100).forEach(post => {
                    ...
                    destinations.push({ kind: 'post', label: post.title || 'Bài viết', detail: post.propertyName || '', url, source: url, icon: '¶' });
                });
            } catch { }
            return destinations;
        })();
    ...
    function destinationLabel(kind) { return ({ system: 'Trang hệ thống', room: 'Phòng', post: 'Bài viết', anchor: 'Trong trang' })[kind] || 'Khác'; }

    function currentStatus(target, input, destinations) {
        const source = sourceValue(target);
        const value = input.value.trim();
        const found = (destinations || []).find(item => item.source === source && item.url === value) || (destinations || []).find(item => item.url === value);
    ...
    function optionHtml(item, index) {
        return `<button type="button" class="pve-link-destination" data-link-destination="${index}"><span class="pve-link-destination-icon">${h(item.icon || (item.kind === 'room' ? '▣' : item.kind === 'post' ? '¶' : '↗'))}</span><span><strong>${h(item.label)}</strong><small>${h(item.detail || item.url)}</small></span></button>`;
    }
    ...
    function renderDestinations(picker, destinations, query) {
        ...
        const indexed = destinations.map((item, index) => ({ item, index })).filter(({ item }) => !needle || `${item.label} ${item.detail} ${item.url} ${destinationLabel(item.kind)}`.toLowerCase().includes(needle));
        if (!indexed.length) { container.innerHTML = '<div class="pve-link-empty">Không tìm thấy liên kết phù hợp.</div>'; return; }
        container.innerHTML = ['system', 'room', 'post', 'anchor'].map(kind => {
            const groupItems = indexed.filter(x => x.item.kind === kind);
            return groupItems.length ? `<section class="pve-link-group"><h4>${h(destinationLabel(kind))}<span>${groupItems.length}</span></h4><div>${groupItems.map(x => optionHtml(x.item, x.index)).join('')}</div></section>` : '';
        }).join('');
    }
    ...
    function buildPicker(target, input) {
        ...
        picker.innerHTML = `<div class="pve-smart-link-summary"><div><strong>Liên kết</strong><span data-link-status>Đang kiểm tra…</span></div><button type="button" data-link-open>⌕ Chọn link có sẵn</button></div><div class="pve-smart-link-panel" data-link-panel hidden><div class="pve-smart-link-search"><input type="search" data-link-search placeholder="Tìm Trang chủ, phòng, bài viết…"><button type="button" data-link-panel-close aria-label="Đóng">×</button></div><div class="pve-link-loading" data-link-loading>Đang tải các link có sẵn…</div><div class="pve-smart-link-destinations" data-link-destinations></div><div class="pve-smart-link-custom"><strong>Link tùy chỉnh</strong><small>Bạn vẫn có thể nhập trực tiếp vào ô Link phía trên nếu cần URL ngoài hệ thống.</small></div></div><div class="pve-system-link-actions">${defaultToken ? `<button type="button" data-link-reset="${defaultToken}">↺ Khôi phục mặc định</button>` : ''}<button type="button" data-link-test>↗ Mở thử</button></div>`;
        ...
        picker.querySelector('[data-link-open]').addEventListener('click', async () => {
            panel.hidden = false;
        ...
        picker.querySelector('[data-link-panel-close]').addEventListener('click', () => { panel.hidden = true; picker.classList.remove('is-open'); });
    ...
    function enhance(drawer) {
        ...
        drawer.querySelectorAll('[data-token-source]').forEach(target => buildPicker(target, target.querySelector('input[name="headerCtaUrl"], input[name="footerBookingUrl"]')));
    }
    ...
    function queue() {
        if (queued) return;
        queued = true;
        requestAnimationFrame(() => { queued = false; document.querySelectorAll('.pve-shell-rates-drawer').forEach(enhance); });
    }
