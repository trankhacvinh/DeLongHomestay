(function () {
    ...
    function defaultRow() {
        return {
            builderKind: 'row', rowVersion: 2, layout: '2-equal', gap: 'md', align: 'top',
    ...
    class RowBuilder {
        ...
        enhanceCoreDrawer() {
            ...
            shortcut.innerHTML = '<strong>Row / Column Builder</strong><span>Tạo 1–4 cột và thêm nhiều phần tử bên trong.</span>';
        ...
        enhanceRowSections() {
            document.querySelectorAll('.pve-editable-section[data-pve-section-id]').forEach(sectionEl => {
                ...
                if (!section) return;
                if (!sectionEl.classList.contains('pve-row-builder-section')) sectionEl.classList.add('pve-row-builder-section');
        ...
        open(section) {
            ...
            drawer.innerHTML = `<header><div><small>ROW / COLUMN BUILDER</small><h2>${section ? h(section.name || 'Chỉnh Row') : 'Tạo Row mới'}</h2></div><button type="button" data-close aria-label="Đóng">×</button></header>
                <form data-row-form><div class="pve-drawer-body">
                    ...
                    <section class="pve-row-preview-wrap"><div class="pve-row-preview-head"><strong>Xem trước nhanh</strong><small>Server vẫn sanitize HTML khi lưu.</small></div><div class="pve-row-preview" data-row-preview></div></section>
        ...
        renderColumns() {
            ...
            root.innerHTML = this.state.columns.map((column, colIndex) => `<article class="pve-row-column" data-column="${colIndex}">
                ...
                <div class="pve-row-add-element">${select(`addKind${colIndex}`, 'text', Object.entries(ELEMENTS))}<button type="button" data-add-element="${colIndex}">＋ Thêm phần tử</button></div>
            </article>`).join('');
        ...
        elementCard(element, colIndex, elementIndex) {
            ...
            const canRight = colIndex < this.state.columns.length - 1;
            const actions = `<div><button type="button" data-element-action="up" data-col="${colIndex}" data-index="${elementIndex}" title="Đưa lên">↑</button><button type="button" data-element-action="down" data-col="${colIndex}" data-index="${elementIndex}" title="Đưa xuống">↓</button>${canLeft ? `<button type="button" data-element-action="left" data-col="${colIndex}" data-index="${elementIndex}" title="Sang cột trái">←</button>` : ''}${canRight ? `<button type="button" data-element-action="right" data-col="${colIndex}" data-index="${elementIndex}" title="Sang cột phải">→</button>` : ''}<button type="button" data-element-action="duplicate" data-col="${colIndex}" data-index="${elementIndex}" title="Nhân bản">⧉</button><button type="button" class="danger" data-element-action="delete" data-col="${colIndex}" data-index="${elementIndex}" title="Xóa">×</button></div>`;
        ...
        bindColumnEvents() {
            ...
            this.drawer.querySelectorAll('[data-add-element]').forEach(button => button.addEventListener('click', () => {
                ...
                this.renderColumns(); this.updatePreview();
            }));
            ...
            this.drawer.querySelectorAll('[data-element-action]').forEach(button => button.addEventListener('click', () => {
                ...
                if (action === 'delete') list.splice(index, 1);
                    ...
                    const target = this.state.columns[targetCol]?.elements;
        ...
        async upload(event) {
            ...
            const col = Number(event.target.dataset.col), index = Number(event.target.dataset.index);
            event.target.value = '';
            ...
            try {
                ...
                this.renderColumns(); this.updatePreview(); this.toast('Đã tải ảnh. Bấm Lưu Row để áp dụng.');
        ...
        async save() {
            ...
            if (payload.contentJson.length > 28000) {
                submit.disabled = false; submit.textContent = 'Lưu Row';
                return this.toast('Row quá lớn. Hãy tách thành hai Row.', true);
            }
            try {
    ...
    DeLongApi.get(contextUrl).then(context => {
        if (context?.canEdit) new RowBuilder(context).mount();
    }).catch(() => {});
})();
