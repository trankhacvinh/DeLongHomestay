(function () {
    ...
    const openButton = root?.querySelector('[data-quick-rate-open]');
    if (!root || !dataNode || !openButton || !window.DeLongApi) return;

    const injectedButton = root.querySelector('.quick-rate-open');
    const injectedModal = document.querySelector('.quick-rate-backdrop');
    if (injectedButton && injectedModal) {
        injectedButton.hidden = true;
        injectedButton.style.display = 'none';
        openButton.addEventListener('click', () => injectedButton.click());
        return;
    }
    ...
    style.textContent = `
        .settings-title-actions{display:flex;align-items:center;justify-content:flex-end;gap:10px;flex-wrap:wrap}
        ...
        .quick-rate-summary{padding:10px 12px;border-radius:10px;background:#f2f8f6;color:#57706a;font-size:10px;line-height:1.45}.quick-rate-summary strong{color:#23534d}.quick-rate-progress{min-height:18px;color:#607772;font-size:10px}.quick-rate-error{color:#a53d37}
    ...
    modal.innerHTML = `
        <section class="quick-rate-card" role="dialog" aria-modal="true" aria-labelledby="quick-rate-title">
            <header class="quick-rate-head">
                <div><span class="quick-rate-kicker">THIẾT LẬP NHANH</span><h2 id="quick-rate-title">Tạo bộ khung giờ</h2><p>Khai báo một lần rồi áp dụng cho nhiều phòng. Khung đã tồn tại ở phòng sẽ được bỏ qua.</p></div>
            ...
            <footer class="quick-rate-foot"><div class="quick-rate-foot-copy">Sau khi tạo, bạn vẫn có thể sửa giá riêng cho từng phòng.</div><div class="quick-rate-foot-actions"><button type="button" class="btn btn-light" data-quick-close>Hủy</button><button type="button" class="btn btn-primary" data-quick-save>Tạo bộ khung giờ</button></div></footer>
        </section>`;
    ...
    function syncSummary() {
        ...
        summary.innerHTML = `<strong>${roomCount} phòng · ${rows.length} khung</strong> · tối đa ${roomCount * rows.length} mức giá mới. Khung trùng loại và giờ sẽ được bỏ qua.`;
    }
    ...
    function seedRows() {
        ...
        rows = template.length > 0
            ? template.map(rate => createRow(rate))
            : [createRow({ name: 'Khung 1', startTime: '11:00', endTime: '14:00', price: 0 })];
        renderRows();
    }
    ...
    async function save() {
        ...
        for (const room of targets) {
            ...
            for (let index = 0; index < rows.length; index += 1) {
                ...
                try {
                    const payload = { name: row.name, startTime: row.startTime, endTime: row.endTime, type: Number(row.type), price: Number(row.price || 0), sortOrder: index };
                    await DeLongApi.post(`/api/admin/properties/${propertyId}/rooms/${room.id}/rates`, payload);
                    created += 1;
                    existing.push({ ...payload, isActive: true });
                } catch {
                    failed += 1;
                }
            }
    ...
    modal.querySelector('[data-quick-load]').addEventListener('click', () => {
        ...
        rows = activeTemplateRates(source).map(rate => createRow(rate));
        progress.textContent = `Đã nạp ${rows.length} khung từ ${source.name}. Bạn có thể sửa trước khi tạo.`;
        progress.classList.remove('quick-rate-error');
        renderRows();
    });
